...
public:
    ...
    bool Load(const std::string& a_Tileset, sf::Vector2u a_TileSize, unsigned int a_Width, unsigned int a_Height)
    {
        ...
        m_Vertices.resize(a_Width * a_Height * 4);

        // populate the vertex array, with one quad per tile
        for (unsigned int i = 1; i < a_Height - 1; ++i)
            for (unsigned int j = 1; j < a_Width - 1; ++j)
            {
                std::pair<int, int> coord;
                if (m_Tiles[i * m_Width + j] == ETILE_TYPE::TILE_FLOOR) { coord = { 0, 0 }; }
                else if (m_Tiles[i * m_Width + j] == ETILE_TYPE::TILE_WALL) { coord = { 0, 0 }; }
                else
                {
...
private:
    ...
    {
        return m_Tiles[a_Y * m_Width + a_X] == ETILE_TYPE::TILE_INTERNAL_WALL || m_Tiles[a_Y * m_Width + a_X] == ETILE_TYPE::TILE_HALF_WALL;
    }
